    ...
    <div class="operate">
        <div class="left">
            <?php if(ACTION_NAME == "index"): ?><input type="button" onclick="goUrl('<?php echo U(GROUP_NAME. '/Article/add', array('pid'=>$pid));?>')" class="btn_blue" value="添加文章">
                <input type="button" onclick="toGetSubmit('<?php echo U(GROUP_NAME.'/Article/move', array('pid' => $pid));?>', <?php echo C('URL_MODEL');?>, '<?php echo C('URL_PATHINFO_DEPR');?>')" class="btn_blue" value="移动">
    ...
    <form action="<?php echo U(GROUP_NAME.'/Article/del');?>" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($vlist)): foreach($vlist as $key=>$v): ?><tr>
                ...
                <a href="javascript:;" onclick="toConfirm('<?php echo U(GROUP_NAME. '/Article/del',array('id' => $v['id'], 'pid' => $pid), '');?>', '确实要删除吗？')">删除</a>
                <?php else: ?>
                <a href="<?php echo U(GROUP_NAME. '/Article/restore',array('id' => $v['id'], 'pid' => $pid), '');?>">还原</a>
                <a href="javascript:;" onclick="toConfirm('<?php echo U(GROUP_NAME. '/Article/clear',array('id' => $v['id'], 'pid' => $pid), '');?>', '确实要彻底删除吗？')">彻底删除</a><?php endif; ?>
				</td>
